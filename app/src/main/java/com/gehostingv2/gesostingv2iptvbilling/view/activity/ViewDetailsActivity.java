...
//        initialize();
    ...
    private void initialize() {
        ...
        if (username != null && password != null &&
            ...
            startViewingDetails(context, username, password);
        } else {
            Intent loginIntent = new Intent(this, LoginWelcomeActivity.class);
    ...
    private void startViewingDetails(Context context,
                                     String username,
                                     String password) {
        vodPresenter = new VodPresenter(this, context);
        Intent intetGetIntent = getIntent();
        if (intetGetIntent != null) {
            streamId = Integer.parseInt(intetGetIntent.getStringExtra(AppConst.STREAM_ID));
            movieName = intetGetIntent.getStringExtra("movie");
            selectedPlayer = intetGetIntent.getStringExtra("selectedPlayer");
            streamType = intetGetIntent.getStringExtra("streamType");
            containerExtension = intetGetIntent.getStringExtra("containerExtension");
            categoryId = intetGetIntent.getStringExtra("categoryID");
            num = intetGetIntent.getStringExtra("num");
            name = intetGetIntent.getStringExtra("name");

            ArrayList<FavouriteDBModel> checkFavourite = database.checkFavourite(streamId, categoryId, "vod");
            if (checkFavourite.size() > 0) {
                tvAddToFav.setText("Remove from Favourite");
                ivFavourite.setVisibility(View.VISIBLE);
            } else {
                tvAddToFav.setText("Add To Favourite");
...
//            viewDetailsActivityIntent.putExtra("containerExtension", containerExtension);
            ...
            if (streamId != -1 && streamId != 0) {
                vodPresenter.vodInfo(username, password, streamId);
            }
        }
    }
...
//        listDataChild.put(listDataHeader.get(6), servicesList);
//        listDataChild.put(listDataHeader.get(7), billingList);
//        listDataChild.put(listDataHeader.get(8), supportList);
//
//    }
...
//    @Override
//    public void vodInfo(VodInfoCallback vodInfoCallback, VodAdapter.MyViewHolder holder) {
//
//    }
...
//                    editor1.putString(AppConst.LOGIN_PREF_ALLOWED_FORMAT, allowedFormat);
    ...
    public void vodInfo(VodInfoCallback vodInfoCallback) {
        if (vodInfoCallback != null) {
            ...
            tvdetailprogressbar.setVisibility(View.GONE);


            if (context != null && movieImage != null && !movieImage.isEmpty())
                Picasso.with(context).load(movieImage).placeholder(R.drawable.noposter).into(ivMovieImage);
            if (movieName != null)
                tvMovieName.setText(movieName);

            if (tvReleaseDateInfo != null && releaseDate != null && !releaseDate.isEmpty() && !releaseDate.equals("n/A") && releaseDate!="n/A") {
                tvReleaseDate.setVisibility(View.VISIBLE);
                tvReleaseDateInfo.setText(releaseDate);
            }
            else
                tvReleaseDateInfo.setText("");
            if (tvDirectorInfo != null && movieDirector != null && !movieDirector.isEmpty() && !movieDirector.equals("n/A")) {
                tvDirector.setVisibility(View.VISIBLE);
                tvDirectorInfo.setText(movieDirector);
            }
            else
                tvDirectorInfo.setText("");
//            if (tvCastInfo != null && cast != null && !cast.isEmpty())
//                tvCastInfo.setText(cast);
//            else
//                tvCastInfo.setText("NA");

            if (tvRatingLabel != null && rating != null && !rating.isEmpty() && !rating.equals("n/A")) {
                tvRating.setVisibility(View.VISIBLE);
                tvRatingLabel.setText(rating);
            }
            else
                tvRatingLabel.setText("");
            if (tvMovieInfo != null && description != null && !description.isEmpty() && !description.equals("n/A") )
                tvMovieInfo.setText(description);
//            else if (tvMovieInfo != null && description != null && !description.isEmpty())
//                tvMovieInfo.setText(description);
            if (tvMovieGenere != null && gnere != null && !gnere.isEmpty()) {
                tvMovieGenere.setVisibility(View.GONE);
                tvMovieGenere.setText(gnere);
            }
        }
    }
    ...
    private void addToFavourite() {
        ...
        tvAddToFav.setText(getResources().getString(R.string.remove_from_favourite));
        ivFavourite.setVisibility(View.VISIBLE);
    }

    private void removeFromFavourite() {
        database.deleteFavourite(streamId, categoryId, "vod");
        tvAddToFav.setText(getResources().getString(R.string.add_to_favourite));
        ivFavourite.setVisibility(View.INVISIBLE);
    }
