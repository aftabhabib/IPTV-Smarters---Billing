...
import com.gehostingv2.gesostingv2iptvbilling.model.pojo.CurrencyPojo;
import com.gehostingv2.gesostingv2iptvbilling.presenter.ClientDetailPresenter;
import com.gehostingv2.gesostingv2iptvbilling.presenter.CurrenciesPresenter;
...
public class ServicesActivity extends AppCompatActivity implements ClientDetailInterface,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currenciesPresenter.getClientCurrencies();
...
//                setupDrawerContent(navView);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            ServicesActivity activity = this;
...
//        billingList.add("MY CREDITS");
    ...
    private void homeFragmentToLoad() {
        if (getSupportFragmentManager().findFragmentByTag(AppConst.TAG_SUBSCRIPTION_PRODUCT_SERVICES) != null) {
            getSupportFragmentManager().popBackStackImmediate(AppConst.TAG_SUBSCRIPTION_PRODUCT_SERVICES,
...
//                    editor1.putString(AppConst.LOGIN_PREF_ALLOWED_FORMAT, allowedFormat);
    ...
    @Override
    public void getCurrencies(GetClientCurrencyCallback getClientCurrencyCallback) {
        loginPreferencesEmail = getSharedPreferences(AppConst.SHARED_PREFERENCE_WHMCS, MODE_PRIVATE);
        currency = (loginPreferencesEmail.getInt(AppConst.PREF_CURRENCIES_WHMCS, 0));
        loginPreferencesClientidEditor = loginPreferencesEmail.edit();
        ArrayList<CurrencyPojo> currenciesPojoArrayList = getClientCurrencyCallback.getCurrenciesListCallback().getCurrencyList();
        String currencyPrefix = "";
        String currencySuffix = "";
        for (CurrencyPojo currencyPojoListItem : currenciesPojoArrayList) {
            if (String.valueOf(currency).equals(currencyPojoListItem.getId())) {
                currencyPrefix = currencyPojoListItem.getPrefix();
                currencySuffix = currencyPojoListItem.getSuffix();
                loginPreferencesClientidEditor.putString(AppConst.PREF_CURRENCIES_PREFIX_WHMCS, currencyPrefix);
                loginPreferencesClientidEditor.putString(AppConst.PREF_CURRENCIES_SUFFIX_WHMCS, currencySuffix);
                loginPreferencesClientidEditor.commit();
            }
        }
