 ...
 * {@link PendingServicesFragment.OnFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link PendingServicesFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class PendingServicesFragment extends Fragment implements ServicesInterface {
    ...
    public void getMyServices(GetClientProductsCallback getClientProductsCallback) {
        if (getClientProductsCallback != null) {
            ...
            ArrayList<ProductDetailPojo> pendingServiceArrayList = new ArrayList<ProductDetailPojo>();


            for (ProductDetailPojo productDetailPojo : productDeailArrayList) {
                String status = productDetailPojo.getStatus();
                if (status.equals("Pending"))
                    pendingServiceArrayList.add(productDetailPojo);
            }
            if (pendingServiceArrayList.size() > 0) {
                myRecyclerView.setVisibility(View.VISIBLE);
                emptyView.setVisibility(View.GONE);
                mAdapter = new ServicesAdapter(pendingServiceArrayList, getContext(), dashboardActivity);
                myRecyclerView.setAdapter(mAdapter);
            } else if (pendingServiceArrayList.isEmpty()) {
                if (myRecyclerView != null && emptyView != null) {
                    myRecyclerView.setVisibility(View.GONE);
                    emptyView.setVisibility(View.VISIBLE);
                    if (context != null)
                        emptyView.setText(context.getResources().getString(R.string.no_pending_services));
                }
            }
        }
    }
