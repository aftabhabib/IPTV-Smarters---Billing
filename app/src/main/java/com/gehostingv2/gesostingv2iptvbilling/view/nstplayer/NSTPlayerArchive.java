...
//        handler.removeMessages(MESSAGE_FADE_OUT);
    ...
    public void play(String url, int opened_stream_id, String videoExtension, String stream_start_time, String stream_duration, String stream_stop_time) {
        ...
        if (playerSupport) {
            $.id(R.id.app_video_loading).visible();
//            String stream_minutes = "0";
//
//            if(!stream_duration.equals("")){
//                int stream_duration_int = Integer.parseInt(stream_duration);
//                stream_minutes = String.valueOf(stream_duration_int * 60);
//            }
            videoView.setVideoPath(url+stream_stop_time+"/"+stream_start_time+"/"+opened_stream_id+"."+videoExtension);
            videoView.start();
        }
    }
...
//            searchEditText.setVisibility(View.GONE);
    ...
    /**
     * using constants in NSTPlayerArchive,eg: NSTPlayerArchive.SCALETYPE_FITPARENT
     * @param scaleType
     */
...
//                    return true;
//                }else
                if(topBoxLinearLayout.getVisibility() == View.VISIBLE){
