...
public class TimeFormatActivity extends AppCompatActivity implements LoginIPTVInterface,
...
//        xmltvPresenter = new XMLTVPresenter(, context);
        loginPreferencesAfterLogin = getSharedPreferences(AppConst.LOGIN_PREF_TIME_FORMAT, MODE_PRIVATE);
        String timeFormat = loginPreferencesAfterLogin.getString(AppConst.LOGIN_PREF_TIME_FORMAT, "");
        if (timeFormat.equals("hh:mm a")) {
            rb12hr.setChecked(true);
        } else {
            rb24hr.setChecked(true);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            TimeFormatActivity activity = this;
            ...
            case R.id.bt_save_changes:
                ...
                if (loginPrefsEditor != null) {
                    if (selectedPlayer.getText().toString().equals(getResources().getString(R.string.normal_time))) {
                        loginPrefsEditor.putString(AppConst.LOGIN_PREF_TIME_FORMAT, "hh:mm a");
                    } else {
                        loginPrefsEditor.putString(AppConst.LOGIN_PREF_TIME_FORMAT, "HH:mm");
                    }
                    loginPrefsEditor.commit();
                    Toast.makeText(TimeFormatActivity.this, getResources().getString(R.string.player_setting_save), Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(TimeFormatActivity.this, getResources().getString(R.string.player_setting_error), Toast.LENGTH_SHORT).show();
                }
