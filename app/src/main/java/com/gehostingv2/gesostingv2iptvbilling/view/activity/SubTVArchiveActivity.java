...
import com.gehostingv2.gesostingv2iptvbilling.view.adapter.SubTVArchiveCategoriesAdapter;
...
public class SubTVArchiveActivity extends AppCompatActivity implements LoginIPTVInterface,
...
//    AppBarLayout appbarToolbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sub_tvarchive);
...
//        Toast.makeText(this, "Opened Stream ID: " + opened_stream_id, Toast.LENGTH_SHORT).show();
        liveStreamDBHandler = new LiveStreamDBHandler(context);
//        Toast.makeText(this, "Opened Channel ID: " + opened_channel_id, Toast.LENGTH_SHORT).show();
...
//        liveStreamsEpgPresenter = new LiveStreamsEpgPresenter(SubTVArchiveActivity.this, context);
//        liveStreamsEpgPresenter.liveStreamsEpg(username, password, opened_stream_id);
    }
    ...
    public void getEPG(String opened_channel_id, int opened_stream_id, String opened_num, String opened_channel_name, String opened_channel_icon, String opened_channel_duration){
        if (liveStreamDBHandler != null) {
            ArrayList<XMLTVProgrammePojo> xmltvProgrammePojos = liveStreamDBHandler.getEPG(opened_channel_id);
            if (xmltvProgrammePojos != null) {
                ...
                if (EpgSize != 0) {
                    ...
                    List<String> datesTabs = new ArrayList<String>();
                    int datesTabsIndex = 0;
                    int currentDateIndex = 0;

//                    for (int j = 0; j < EpgSize; j++) {
//                        singleDateTime = xmltvProgrammePojos.get(j).getStart();
//                        Long epgDateToTimestamp = Utils.epgTimeConverter(singleDateTime);
//                        String currentFormatDate = new SimpleDateFormat("dd-MMM-yyyy", Locale.US).format(epgDateToTimestamp);
...
//                    String singleDateTime;
                    ...
                    for (int k = 0; k < EpgSize; k++) {
                        singleDateTime = xmltvProgrammePojos.get(k).getStart();
                        Long epgDateToTimestamp = Utils.epgTimeConverter(singleDateTime);
                        String currentFormatDate = new SimpleDateFormat("dd-MMM-yyyy", Locale.US).format(epgDateToTimestamp);
                        Long dateDiff = getDateDiff(simpleDateFormat,currentFormatDate,currentFormatDateAfter);
                        if(dateDiff>=0){
                            if(!datesTabs.contains(currentFormatDate)){
                                datesTabs.add(datesTabsIndex,currentFormatDate);
                                if (currentFormatDateAfter.equals(currentFormatDate)) {
...
//                    Set<String> hashSetDates = new HashSet<String>(Arrays.asList(datesArray));
//                    List<String> sortedDates = new ArrayList<String>(hashSetDates);
//                    List<String> sortedDatesFutureRemoved = new ArrayList<String>();
...
//                    int index = 0;
////                String sortedDatesArray[] = new String[sortedDatesSize];
//                    for (String sortedDate : sortedDates) {
//                        sortedDatesFutureRemoved.add(index,sortedDate);
//                        if (currentFormatDate.equals(sortedDate)) {
//                            currentDateIndex = index;
//                            break;
//                        }
////                        sortedDatesArray[index] = newDateFormat.format(oldDateFormat.parse(sortedDate));
//                        index++;
//                    }
                    viewpager.setAdapter(new SubTVArchiveCategoriesAdapter(datesTabs, xmltvProgrammePojos,opened_stream_id,opened_num,opened_channel_name,opened_channel_icon,opened_channel_id,opened_channel_duration,getSupportFragmentManager(),
                            SubTVArchiveActivity.this));
                    slidingTabs.setupWithViewPager(viewpager);
                    viewpager.setCurrentItem(currentDateIndex);
                    ...
                    if(rlTvArchiveTitle!=null){
                        rlTvArchiveTitle.setVisibility(View.VISIBLE);
                    }
                    if(tvNoRecordFound!=null){
                        tvNoRecordFound.setVisibility(View.VISIBLE);
                    }
                    if(tvEpgRequired!=null){
                        tvEpgRequired.setVisibility(View.VISIBLE);
                    }
                }
                onFinish();
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            SubTVArchiveActivity activity = this;
